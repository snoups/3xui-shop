...
class SubscriptionService:
    ...
    async def is_trial_available(self, user: User) -> bool:
        is_first_check_ok = (self.config.shop.TRIAL_ENABLED
    ...
    async def gift_trial(self, user: User) -> bool:
        """
        Assigning 'Try for free' trial bonus days to a new user.
        Calls VPN Service to process assigning bonus days for new user and calls database method to log this action.

        Args:
            user (User): User object.

        Returns:
            bool: True, when the trial has been successfully given, else False.
        """

        if not await self.is_trial_available(user=user):
            logger.warning(f"Failed to activate trial for user {user.tg_id}. Trial period is not available.")
            return False
        ...
        if trial_success:
            logger.info(f"Successfully gave {self.config.shop.TRIAL_PERIOD} days to a user {user.tg_id}")
            return True
