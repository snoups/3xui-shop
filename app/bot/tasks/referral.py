...
async def reward_pending_referrals_after_payment(session_factory: async_sessionmaker, referral_service: ReferralService):
    async with session_factory() as session:
        ...
        for reward in pending_rewards:
            ...
            if not success:
                logger.warning(f"[Background check] Reward {reward.id} was NOT proceed successfully.")

        logger.info("[Background check] Referrer rewards check finished.")


def start_referral_scheduler(session_factory: async_sessionmaker, referral_service: ReferralService):
